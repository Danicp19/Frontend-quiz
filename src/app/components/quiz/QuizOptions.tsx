...
function QuizOptions({ option, optionIndex, userAnswer, setIsAnswered }: QuizOptionsProps) {
    const { chosenAnswer, correctAnswer } = useSelector((state: RootState) => state.quiz);
    ...
    return (
      ...
      `}
            onClick={choosenAnswer}
        >
            <span
                className={`inline-block rounded-xl p-6 transition-all duration-300  ${isAnswerCorrect
                    ? "bg-green text-white"
                    : isAnswerWrong
                        ? "bg-red text-white"
                        : isOptionChosen
       ...
       `}
            ...
            <span
                className={` ${darkMode ? "text-white" : "text-dark-navy"
